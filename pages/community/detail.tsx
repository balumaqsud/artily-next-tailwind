	...
	const {
		...
		onCompleted: (data: T) => {
			...
			if (data?.getBoardArticle?.memberData?.memberImage)
				setMemberImage(`${process.env.REACT_APP_API_GRAPHQL_URL}/${data?.getBoardArticle?.memberData?.memberImage}`);
		},
	});
							...
							<div className="config">
								<Stack className="first-box-config">
									...
									<Stack className="like-and-dislike">
										<Stack className="top">
											<Button onClick={() => likeArticleHandler(user, boardArticle?._id as string)}>
